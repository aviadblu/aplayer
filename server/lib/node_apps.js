...
//	}
	 ...
	 */
	//Oleg: Fix is a workaround to enable restart
	var _startApp = function(args, cwd, hidden) {
		...
		try {
			...
			//workaround to enable restart
			if (appTable.apps().length)
			{
				console.log("applicaiton already running");
				var rApp = appTable.apps()[0]
				console.log("stopping app " + rApp.pid)
				rApp.emit("stdout", "stopping app " + rApp.pid + "restarting ...\r\n");
				appTable.remove(rApp.pid);
				rApp.process.kill();
			}
...
// TODO - configParams
exports.AppContext = new AppContext({fileRoot: '/file', workspaceDir: '/workspace', configParams: {}});
exports.App = App;
